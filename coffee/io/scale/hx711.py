# From https://github.com/tatobari/hx711py/blob/master/hx711v0_5_1.py
...
class HX711:
    ...
    def setChannel(self, channel: str = "A"):
        if channel == "A":
            self.setGain(128)
            return True
        elif channel == "B":
    ...
    def readRawBytes(self, blockUntilReady: bool = True):
    ...
    def getRawBytes(self, channel: str = "A"):
        # Get current channel
    ...
    def readyCallback(self, pin: int):
    ...
    def enableReadyCallback(
        self, paramCallback: Optional[Callable[[List[int]], None]] = None
    ):
        self.paramCallback = (
    ...
    def setReadingFormat(self, byteFormat: str = "MSB", bitFormat: str = "MSB"):
    ...
    def rawBytesToLong(self, rawBytes: Optional[bytes] = None):
        if rawBytes is None:
            return None
    ...
    def getLong(self, channel: str = "A"):
        currentChannel = self.getChannel()
        if channel != currentChannel:
            self.setChannel(channel)
    ...
    def setOffset(self, offset: float, channel: str = "A"):
        if channel == "A":
    ...
    def setOffsetA(self, offset: float):
        return self.setOffset(offset, "A")

    def setOffsetB(self, offset: float):
        return self.setOffset(offset, "B")

    def getOffset(self, channel: str = "A"):
        if channel == "A":
    ...
    def rawBytesToLongWithOffset(
        self, rawBytes: Optional[bytes] = None, channel: str = "A"
    ):
        if rawBytes is None:
            return None
    ...
    def getLongWithOffset(self, channel: str = "A"):
        currentChannel = self.getChannel()
        if channel != currentChannel:
            self.setChannel(channel)
    ...
    def setReferenceUnit(self, referenceUnit: float, channel: str = "A"):
        if channel == "A":
    ...
    def getReferenceUnit(self, channel: str = "A"):
        if channel == "A":
    ...
    def rawBytesToWeight(self, rawBytes: Optional[bytes] = None, channel: str = "A"):
        if rawBytes is None:
            return None
    ...
    def getWeight(self, channel: str = "A"):
        currentChannel = self.getChannel()
        if channel != currentChannel:
            self.setChannel(channel)
    ...
    def autosetOffset(self, channel: str = "A"):
